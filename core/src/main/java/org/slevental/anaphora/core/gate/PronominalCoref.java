...
import org.slevental.anaphora.core.gate.lazy.LazyAnnotationSet;
import org.slevental.anaphora.core.gate.lazy.LazyComputation;
import org.slevental.anaphora.core.triplet.Triplet;
...
public class PronominalCoref extends AbstractLanguageAnalyser
    ...
    private static final String NP_CATEGORY = "NP";
    ...
    public PronominalCoref() {
        this.tripletExtractor = new TripletExtractor(false);
...
//System.out.println("PLEONASM...");
        ...
        //2. try to find named entity which corresponds to $IT$ITS$ITSELF
        while (currSentenceIndex >= scopeFirstIndex) {

            Sentence currSentence = this.textSentences[currSentenceIndex];
            Set<Annotation> org_loc = new TreeSet<Annotation>(ANNOTATION_OFFSET_COMPARATOR);
            org_loc.addAll(currSentence.getInanimated());

            if (currSentenceIndex > 0) { // get objects from previous sentence
                org_loc.addAll(this.textSentences[currSentenceIndex - 1].getObjects());
            }
...
//System.out.println("Oops! ["+pronounString+"] not part of quoted fragment...");
     ...
     * ---
     */
    private void preprocess() throws ExecutionException, InvalidOffsetException {
        ...
        for (int i = 0; i < sentenceArray.length; i++) {
            ...
            String content = defaultAnnotations.getDocument().getContent()
                    .getContent(sentStartOffset, sentEndOffset).toString();
            ...
            //4.3 get noun phrases
            AnnotationSet objects = getObjects(content, sentStartOffset, tempASOffsets);
    ...
    private AnnotationSet getObjects(final String currSentence, final Long offset, final AnnotationSet annSet) {
        return new LazyAnnotationSet(new LazyComputation<AnnotationSet>() {
            @Override
            public AnnotationSet compute() throws InvalidOffsetException {
                Triplet triplet = tripletExtractor.extract(currSentence);
                if (triplet.getObject() != null) {
                    FeatureMap f = Factory.newFeatureMap();
                    f.put("string", triplet.getObject().getSpan().getCoveredText(currSentence));
                    long start = offset + triplet.getObject().getSpan().getStart();
                    long end = offset + triplet.getObject().getSpan().getEnd();
                    defaultAnnotations.add(start,
                            end,
                            AnnotationType.Object.name(),
                            f);
                    return defaultAnnotations.get(AnnotationType.Object.name(), start, end);
                }
                return new AnnotationSetImpl(defaultAnnotations.getDocument());
            }
        });
    }
...
//    Assert.assertTrue(diff1 != 0 && diff2 != 0);
    ...
    private class Sentence {
        ...
        public AnnotationSet getObjects() {
            return objects;
        }
    }
